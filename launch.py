...
result_path = f"{work_path}{os.sep}results"
...
def update_record(dict, key, value):
...
def _create_config(task_name: str, file_name: str, **kwargs):
    def update_record(dict, key, key_):
        if key_ in kwargs:
            dict[key] = kwargs[key_]

    # Create the result folder.
    folder_dir, index = f"{result_path}{os.sep}{file_name}", 0
    while os.path.exists(f"{folder_dir}_{index}"):
        index += 1
    folder_dir = f"{folder_dir}_{index}"
    os.mkdir(folder_dir)

    # Create the config file.
    import mlora
    template = f"{work_path}{os.sep}{template_path}{os.sep}{kwargs['template']}.json"
    ...
    for lora_template in lora_templates:
        lora_config = lora_template.copy()
        casual_task = task_name not in mlora.tasks.task_dict
        if casual_task:
            lora_config["name"] = f"casual_{index}"
            lora_config["task_name"] = "casual"
            lora_config["data"] = task_name
            ...
            lora_config["name"] = f"{task_name.replace('-', '_')}_{index}"
            lora_config["task_name"] = task_name
    ...
    config_dir = f"{folder_dir}{os.sep}config.json"
    with open(config_dir, "w") as fp:
        json.dump(template_obj, fp, indent=4)
    print(f"Configuration file saved to {config_dir}")
    return folder_dir


def gen_config(tasks: str, config_name: str = "mlora.json", **kwargs):
    root_config = f"{work_path}{os.sep}{config_name}"
    task_dict = {}
    ...
    if "multi_task" in kwargs:
        task_name = ','.join(tasks)
        folder_dir = _create_config(task_name, "multi_tasks", **kwargs)
        task_dict[task_name] = folder_dir
    else:
        for task_name in tasks:
            folder_dir = _create_config(task_name, task_name, **kwargs)
            task_dict[task_name] = folder_dir

    # Save the task_dict to the root config file.
    with open(root_config, "w") as fp:
...
def avail_tasks():
    ...
    for name in mlora.tasks.task_dict.keys(): x
    print(f"    {name}")
